...
public class CreateModeratorController {
    ...
    public void handleCreateModeratorButton(){
        ...
        if(Util.search(userField.getText(),accountList.getAllAccount(),new SearchAccountByUserName()) != null){
            errorLabel.setText("This username is already used"); //duplicate username alert
            return;
        }
        accountList.addAccount(new Moderator(Integer.toString(Integer.parseInt(accountList.getAllAccount().get(accountList.getAllAccount().size()-1).getId())+1) ,userField.getText(),passwordField.getText(),
